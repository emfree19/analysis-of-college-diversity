...
m10 <- get_decennial(geography = "state", variables = "H001001", year = 2010, geometry = TRUE)
state_data <- full_join(m10, joined_data, by = c("NAME" = "state_abbreviation"))

african_american <- state_data %>%
  group_by(NAME) %>%
  mutate(african_american_mean = mean(percent_black_or_african_american, na.rm = TRUE)) %>%
  filter(year == 2018)
